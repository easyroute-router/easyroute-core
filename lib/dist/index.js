...
class UrlParser {
    ...
    static createRouteObject(matchedRoutes, url) {
        ...
        const maxDepth = Math.max(...matchedRoutes.map((route) => route.nestingDepth));
...
function parseRoutes(routes, url) {
    ...
    return routes.filter((route) => {
        if (usedIds.includes(route.id) || usedDepths.includes(route.nestingDepth) || !route.regexpPath.test(url))
            return false;
